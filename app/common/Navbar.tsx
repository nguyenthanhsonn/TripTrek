...
export default function Navbar() {
  ...
  return (
    <nav
      className={`fixed ${isNotHome && 'hidden'} top-0 left-0 w-full bg-[#33333380] backdrop-blur-sm text-white shadow-md z-50 transition-transform duration-300 ease-in-out ${state.isVisible ? 'translate-y-0' : '-translate-y-full'
        }`}
    >
          ...
          <div className="hidden md:flex space-x-8 text-lg font-medium">
            {navItems.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className={`hover:text-sky-400 transition-colors duration-200 ${state.activeItem === item.href ? "text-sky-400" : ""
                  }`}
              >
                {item.name}
              </Link>
          ...
          <div className="flex items-center space-x-3">
            {
              !token ? (
                ...
                <>
                  {/* ⭐ Thêm chuông thông báo tại đây ⭐ */}
                  <NotificationDropdown /> {/* ← thêm vào đây */}
