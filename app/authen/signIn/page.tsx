...
import useAuth from "../../../hooks/useAuth"; // Giả sử useAuth chứa logic đăng nhập
import BackButton from "@/app/common/BackButton";
import { useRouter } from "next/navigation"; // Để sử dụng điều hướng
...
export default function LoginPage() {
  ...
  const { login } = useAuth();
  const router = useRouter(); // Sử dụng hook router

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const user = await login(email, password); // Giả sử login trả về user với thông tin
    if (user) {
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 w-full max-w-5xl bg-white shadow-xl rounded-xl overflow-hidden">
        ...
        <div className="flex flex-col items-center justify-center p-10 bg-gray-100">
          ...
          <Image
            src="/image 47.png" // file nằm trong /public
            alt="Traveler"
        ...
        {/* Right side (Form) */}
        <div className="p-10 flex flex-col justify-center animate-fade-in" style={{ animation: "fadeIn 0.8s ease" }}>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex justify-between text-sm">
              <Link href="/profile/otpCheck" className="text-sky-500 hover:underline">
                Quên mật khẩu?
              </Link>
            </div>
            <button
