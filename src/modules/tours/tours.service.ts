...
export class ToursService {
  ...
  async createTour(dto: CreateTour, user: User) {
    ...
    const newTour = await this.tourRepo.save(tour);

    // Gửi thông báo cho admin biết có tour mới chờ duyệt
    await this.notiService.createAndSendToAdmin({
      userId: null,
      senderId: user.id,
      title: "Tour mới chờ duyệt",
      message: `${creator?.username} vừa đăng một tour mới.`,
  ...
  async getCountTourToday() {
    // Tổng số tour (chưa bị xóa)
    const totalTours = await this.tourRepo.count({
      where: { isDeleted: false },
    });

    // Tour được tạo hôm nay
  ...
  async getTourToDay(): Promise<TourToday[]> {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const tomorrow = new Date(today);
    tomorrow.setDate(tomorrow.getDate() + 1);

    // Query tất cả tour được tạo hôm nay (cho admin xem)
    const tours = await this.tourRepo.find({
      where: {
        createdAt: Between(today, tomorrow),
        isDeleted: false,
    ...
    if (!tours || tours.length === 0) {
      throw new NotFoundException('Không có tour nào được tạo hôm nay');
    }
