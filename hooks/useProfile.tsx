...
export default function useProfile() {
    ...
    const updateProfile = async ({avatar, phone, username}:{avatar:string, phone:string, username:string}) => {
        const url = `${apiUrl}/user/profile-update`;
        const body = { avatar, phone, username };
        console.log("updateProfile ->", url, body, "token:", !!getToken());
      try {
        ...
        console.log("updateProfile response:", response.status, parsed);

        const message = typeof parsed === "string" ? parsed : parsed?.message;
        if(response.ok){
          showSuccess(message || "Cập nhật thành công");
          // try return the updated user object if backend included it
          const updated = typeof parsed === "object" && (parsed.user || parsed.data) ? (parsed.user || parsed.data) : parsed;
          return updated;
        } else {
